...
export const createOpportunity = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const opportunity = await opportunityService.createOpportunity(req.user._id, validatedData);
...
export const getOpportunities = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await opportunityService.getOpportunities({ page, limit, type, search });
